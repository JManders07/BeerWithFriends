      ...
      <div class="w3-container" id="about">
        <div class="w3-content" style="max-width:1200px, position: relative;">
          ...
          <p>
            ...
            <br>
            <strong>General information about {{ beer.name }}:</strong>
            <br>
            {{ beer.description }}
            <br>
            <b-button variant="danger" class="btn" type="submit" @click="deleteBeer(); $router.push({ path:`/beers`})">Delete</b-button>
            <b-button variant="warning"><router-link :to="'/edit/' + `${beer.id}`">Edit</router-link></b-button>
            <br>
            <strong>Given reviews about {{ beer.name }}:</strong>
            <br>
...
export default {
  ...
  mounted() {
    ...
    axios
    .get('https://localhost:7064/api/Beer/' + this.beerid).then((response) => {
      this.beer = response.data;
      console.warn(this.beer);
    }),
    axios
    .get('https://localhost:7064/api/Review/' + this.beerid).then((response) => {
